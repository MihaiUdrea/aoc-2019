...
array<Point, 4> deltaPts = { Point{0, 1, 0}, {0, -1, 0}, {-1, 0, 0}, {1, 0, 0} };
const char* chars = " #._*";
...
struct Solve : Program {
  ...
  void GoTo(Point p)
  {
    ...
    auto& destPar = parentList[p];
    auto& crPar = parentList[lastPoint];

    auto where = find(destPar.begin(), destPar.end(), lastPoint);
    if (where == destPar.end())
    {
      // find a parent on path
      directions.push_back(reverseList[lastPoint]);
      for (int i = crPar.size() - 1; i >= 0; i--)
      {
        auto & parentPt = crPar[i];
        where = find(destPar.begin(), destPar.end(), parentPt);
        if (where == destPar.end())
          directions.push_back(reverseList[parentPt]);
        else
        {
          break;
        }
    ...
    for (where++; where != destPar.end(); where++)
    {
      // forward move reverse the reverse
      directions.push_back(Reverse(reverseList[*where]));
    }

    directions.push_back(Reverse(reverseList[p]));
    ...
    for (auto d : directions)
    {
      ...
      Run(1);
      //assert(output[0] == 1);
      //toConsole(origin + lastPoint, " "s);
      lastPoint = lastPoint + deltaPts[d - 1];
      //toConsole(origin + lastPoint, "D"s);
    }
  }
  ...
  Point Explore(PointData p)
  {
    if (history.insert(p.pos).second == false)
      return {}; /// existing

    GoTo(p.pos);

    assert (lastPoint == p.pos);


    for (auto i : irange(0, 4))
    {
      Direction dir = (Direction)(i + 1);
      Point newPoint = lastPoint + deltaPts[dir - 1];
      ...
      {
        // wall
        toConsole(origin + newPoint, "#"s);

        wals.insert(newPoint);
        history.insert(newPoint);
      ...
      else if (output[0] == 1)
      {
        ...
        space.insert(newPoint);
        que.push({ newPoint, p.dist + 1});
        
        auto parents = parentList[lastPoint];
        parents.push_back(lastPoint);
        parentList.insert({ newPoint, std::move(parents) });

        reverseList.insert({ newPoint, Reverse(dir)});
      ...
      else // 2
      {
        ...
        cout << "Oxygen at: x: " << newPoint.x << " y: " << newPoint.y << " dist: " << minDist;
        return newPoint;
      }

      // step back
      {
        SetPorts({ Reverse(dir) }, {});
        Run(1);
        if (!(output[0] == 1))
          cout << "BAD";
        lastPoint = lastPoint + deltaPts[Reverse(dir) - 1];
        //toConsole(origin + lastPoint, "D"s);
      }
    }    
  ...
  set<Point> space;
  Que que;
  map<Point, vector<Point>> parentList;
  map<Point, Direction> reverseList;
  Point lastPoint;
  ...
  auto Do()
  {
    ...
    while (!que.empty())
    {
      ...
      toConsole({ 2, 2, 0 }, "Time: " + to_string(t));
...
TEST_CASE("New2", "[x.]") {
  toConsole(Point{ 70, 5, 0 }, "Part 1: " + to_string(Solve(ReadFileToString(L"input.txt")).Do()));
